...
function UpperSection() {
  return (
    <div className="svg-bg min-h-[250px] border-b flex gap-3 flex-col items-center justify-center bg-[#112]">
    ...
    <div className="svg-bg-2 w-full min-h-[300px] flex flex-col gap-6 pt-5">
      ...
      <div className="flex gap-4 w-full justify-center items-center flex-wrap">
        <button
          ...
          className=" cursor-pointer font-mono font-semibold py-3 px-4 rounded-full bg-slate-900 text-white flex gap-2 border-2 border-slate-400 hover:bg-slate-700 transition-bg transition-[2s]"
        >
          <SvgCreate />
          Create Your CV
        </button>
        <button
          ...
          className="cursor-pointer font-mono font-semibold py-3 px-6 rounded-full border-2 border-indigo-600 bg-white hover:bg-gradient-to-r from-indigo-500 to-blue-600 hover:border-[#eee] hover:text-white"
        >
          View available Cv templates
        </button>
      </div>
      <h3 className="text-slate-500 w-full text-center my-2 italic">
...
function LowerSection() {
  return (
    <div className="w-full flex flex-col items-center justify-center bg-slate-100">
      <h1 className="text-center w-fit bold text-[1.5rem]  px-4 py-2 text-slate-700  mx-2 my-6 max-[640px]:text-[1.1rem] border-b-2 border-blue-600">
        ...
        {features.map((feature, id) => (
          ...
          >
            <div className="flex gap-2 border-b border-slate-200 p-2 mb-2">
              ...
              <h2 className="text-slate-700/80 bold">{feature.feature}</h2>
            </div>
            <div className="w-full flex-1 flex items-center">
              <p className="px-2 text-slate-500">{feature.description}</p>
            </div>
          </div>
        ))}
      </div>
